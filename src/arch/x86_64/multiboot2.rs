...
impl<'a> BootInfoTag<'a> {
    fn parse(tag_header: &'a BootInfoTagHeader, tag_data: &'a [u8]) -> Option<BootInfoTag<'a>> {
        match tag_header.tag_type {
            x if x == BootInfoTagType::Cmdline as u32 => {
                let cmdline = from_utf8(tag_data).unwrap();
                Some(BootInfoTag::Cmdline(cmdline))
            }
            x if x == BootInfoTagType::BootLoaderName as u32 => {
                let blname = from_utf8(tag_data).unwrap();
            ...
            x if x == BootInfoTagType::Module as u32 => {
                ...
                let mcmd = from_utf8(mod_cmd).unwrap();
...
impl<'a> Iterator for BootInfoReader<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        BootInfoTag::parse(tag_header, tag_data)
    }
}
